<script lang="ts">
	...
	let nextParticleWrapper: HTMLDivElement;
	onMount(async () => {
		const { NextParticle } = await import('../nextparticle.js');
		...
		const nextParticle = new NextParticle({
			...
			initPosition: 'none',
			initDirection: 'none',
			wrapperElement: nextParticleWrapper
		});
...
<nav class="mx-4 py-6">
	...
	>
		<div
			class="flex flex-col items-center space-y-4 sm:flex-row-reverse sm:items-stretch sm:space-x-4 sm:space-y-0 sm:space-x-reverse"
		>
			<a
	...
	<div class="container mx-auto text-center text-3xl font-bold">
		...
		<p>
			<a
				...
				target="_blank"
				rel="noopener noreferrer"
				class="text-sky-400 underline">Follow</a
			> for updates.
		</p>
	</div>
</div>

<div class="mx-4 flex justify-center py-6">
	<img src="images/logo.png" alt="MIDH Logo" class="max-h-16" />
</div>
